...
/**
 * Cronフィールドの定義マップ
 * CronFieldKey（"minute" | "hour" | "day" | "month" | "day_of_week"）をキーとして、
 * 各フィールドの設定にアクセス可能
 *
 * 使用例: CRON_FIELDS["minute"], CRON_FIELDS["hour"]
 */
...
/**
 * Cronフィールドのキー配列（Cron式の順序を明示的に指定）
 * Cron式の順序: 分 時 日 月 曜日
 */
export const CRON_FIELD_KEYS = [
    "minute",
    "hour",
    "day",
    "month",
    "day_of_week",
] as const satisfies readonly CronFieldKey[];
